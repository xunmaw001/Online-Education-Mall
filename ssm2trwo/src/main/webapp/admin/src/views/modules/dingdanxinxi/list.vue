<template>
  <div class="main-content">
        ...
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
            ...
            <el-table-column width="300" :align="contents.tableAlign" 
               :header-align="contents.tableAlign"
                ...
                <el-button v-if="isAuth('dingdanxinxi','发送') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="fufeikechengCrossAddOrUpdateHandler(scope.row,'cross','是','','')">{{ contents.tableBtnFont == 1?'发送':'' }}</el-button>
                <el-button v-if="isAuth('dingdanxinxi','发送') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="fufeikechengCrossAddOrUpdateHandler(scope.row,'cross','是','','')">{{ contents.tableBtnFont == 1?'发送':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('dingdanxinxi','发送') && contents.tableBtnIcon == 0" type="success" size="mini" @click="fufeikechengCrossAddOrUpdateHandler(scope.row,'cross','是','','')">{{ contents.tableBtnFont == 1?'发送':'' }}</el-button>
    ...
    <fufeikecheng-cross-add-or-update v-if="fufeikechengCrossAddOrUpdateFlag" :parent="this" ref="fufeikechengCrossaddOrUpdate"></fufeikecheng-cross-add-or-update>
...
import fufeikechengCrossAddOrUpdate from "../fufeikecheng/add-or-update";
export default {
  data() {
    return {
      searchForm: {
      ...
      fufeikechengCrossAddOrUpdateFlag: false,
  ...
  methods: {
    ...
    fufeikechengCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
	if(crossOptAudit=='是'&&row.sfsh!='是') {
      ...
      this.fufeikechengCrossAddOrUpdateFlag = true;
		...
		for (var o in obj){
		  if(o==statusColumnName && obj[o]==statusColumnValue){
			...
			this.getDataList();
		      ...
		      this.showFlag = true;
		      this.fufeikechengCrossAddOrUpdateFlag = false;
			return;
		  }
		}
	}
      this.$nextTick(() => {
      this.$refs.fufeikechengCrossaddOrUpdate.init(row.id,type);
      });
    },
    payHandler(row){
