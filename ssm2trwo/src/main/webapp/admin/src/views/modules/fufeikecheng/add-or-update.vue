<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'&& !ro.fufeishipin" label="付费视频" prop="fufeishipin">
          <file-upload
          tip="点击上传付费视频"
          action="file/upload"
          :limit="1"
          :multiple="true"
          :fileUrls="ruleForm.fufeishipin?ruleForm.fufeishipin:''"
          @change="fufeishipinUploadChange"
          ></file-upload>
        </el-form-item> 
        <div v-else>
          <el-form-item v-if="ruleForm.fufeishipin" label="付费视频" prop="fufeishipin">
            <el-button type="text" size="small" @click="download(ruleForm.fufeishipin)">预览</el-button>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="付费内容" prop="fufeineirong">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.fufeineirong" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.fufeineirong" label="付费内容" prop="fufeineirong">
                    <span v-html="ruleForm.fufeineirong"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
	...
	this.ruleForm.fufeineirong = this.ruleForm.fufeineirong.replace(reg,'../../../ssm2trwo/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
	...
	if(this.ruleForm.fufeishipin!=null) {
		this.ruleForm.fufeishipin = this.ruleForm.fufeishipin.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `fufeikecheng/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.fufeikechengCrossAddOrUpdateFlag = false;
						 this.parent.search();
    ...
    fufeishipinUploadChange(fileUrls) {
	this.ruleForm.fufeishipin = fileUrls;
	this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
